...
type Tracer(logger: Action<string>, setNrGameObjects: Action<int>) =
    member public this.Log msg = if Object.Equals(logger, null) then logger.Invoke(msg) else ()
    member public this.CountGameObjects nr = if Object.Equals(setNrGameObjects, null) then setNrGameObjects.Invoke(nr) else ()
